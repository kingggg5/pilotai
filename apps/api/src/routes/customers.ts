...
export async function registerCustomerRoutes(app: FastifyInstance, container: AppContainer) {
  ...
  api.post("/api/v1/customer/orders", { preHandler: authenticate, schema: { tags: ["customer"], body: PurchaseRequest, response: { 201: z.any(), 400: z.any(), 401: z.any(), 409: z.any() } } }, async (request, reply) => {
    ...
    await container.audit.fromRequest(request, actor, { action: AuditActions.orderCreated, resourceType: "order", resourceId: order.id, metadata: { item_count: lines.length, quantity: lines.reduce((sum, line) => sum + line.quantity, 0), subtotal, currency: order.currency, ticket_id: item.ticket.id, ai_provider: run.provider, ai_category: run.classification.category, ai_priority: run.classification.priority } });
  ...
  api.get("/api/v1/customer/orders/:orderId", { preHandler: authenticate, schema: { tags: ["customer"], params: z.object({ orderId: z.string().min(1).max(128) }), response: { 200: z.any(), 403: z.any() } } }, async (request, reply) => {
    ...
    if (!linked || linked.order_id?.toLowerCase() !== request.params.orderId.toLowerCase()) return reply.code(403).send({ detail: "Order is not linked to this account", code: "FORBIDDEN" });
    const ownOrder = await container.orders.get(request.params.orderId, actor.tenant_id);
    const result = ownOrder ? { order_id: ownOrder.id, status: ownOrder.status, tracking_number: ownOrder.tracking_number ?? null, estimated_delivery: ownOrder.estimated_delivery ?? null, updated_at: ownOrder.updated_at } : await container.tools.getOrderStatus(request.params.orderId, actor.tenant_id);
