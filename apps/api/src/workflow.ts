...
import { ApprovalRecord, AssistResponse, ClassificationResult, PolicyDecision, RetrievalResult, type AssistRequest } from "./domain.js";
...
const State = new StateSchema({
  ...
  locale: z.string().default("auto"), classification: ClassificationResult.optional(),
...
export class AssistanceWorkflow {
  ...
  constructor(
    ...
    const builder = new StateGraph(State)
      ...
      .addEdge("classify_request", "retrieve_evidence")
      .addEdge("retrieve_evidence", "draft_response")
      .addEdge("draft_response", "check_policy")
      .addConditionalEdges("check_policy", (state) => state.policy!.requires_approval ? "request_approval" : "complete_workflow", { request_approval: "request_approval", complete_workflow: "complete_workflow" })
  ...
  private async draft(state: TicketState) {
    if (!state.retrieval!.sufficient_evidence) {
      const thai = state.locale === "th" || /[\u0E00-\u0E7F]/u.test(state.message);
  ...
  private async approve(state: TicketState) {
    ...
    if (response.decision === "reject") return { approval: response };
    ...
    return { approval: response, escalation_id: escalation.escalation_id };
  }
  ...
  private async response(result: TicketState & Record<string, unknown>, tenantId: string): Promise<AssistResponse> {
    ...
    const status = prompt ? "awaiting_approval" : !result.policy!.allowed ? "refused" : result.escalation_id || result.approval ? "completed" : !result.retrieval!.sufficient_evidence ? "needs_evidence" : "completed";
    const response = AssistResponse.parse({ thread_id: result.thread_id, status, classification: result.classification, retrieval: result.retrieval, draft: result.draft, policy: result.policy, approval: prompt ?? result.approval ?? null, answer: result.answer ?? null, escalation_id: result.escalation_id ?? null, provider: this.languageModel.name });
    await this.runs.save(response, tenantId);
    return response;
  }

  async start(request: AssistRequest, tenantId = "tenant-local", threadId: string = randomUUID()) {
    const result = await this.graph.invoke({ thread_id: threadId, message: request.message, customer_id: request.customer_id, order_id: request.order_id, metadata: request.metadata, locale: request.locale }, this.config(threadId, tenantId));
