...
export default async function Page({ searchParams }: { searchParams: Promise<{ lang?: string; next?: string }> }) {
	...
	return <div className="account-auth-page"><SiteHeader language={language} copy={copy} route="/account/login" /><main className="account-auth-main"><CustomerAuthForm mode="login" language={language} copy={copy} nextPath={nextPath} ssoEnabled={ssoConfigured()} /></main></div>;
}
