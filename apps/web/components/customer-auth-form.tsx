	...
	return (
		<section className="account-auth-card">
			<div className="auth-tabs">
				<Link aria-current={mode === "login" ? "page" : undefined} href={`/account/login?lang=${language}${nextQuery}`}>
					{copy.account.signIn}
				</Link>
				<Link aria-current={mode === "register" ? "page" : undefined} href={`/account/register?lang=${language}${nextQuery}`}>
					{copy.account.register}
				</Link>
			</div>
			...
			<form onSubmit={submit}>
				...
				<label>
					...
					<input name="email" required type="email" maxLength={240} autoComplete="email" />
				</label>

				{mode === "register" && (
					<label>
						<span>{copy.account.phone}</span>
						<input name="phone" required type="tel" minLength={7} maxLength={32} autoComplete="tel" />
					</label>
				)}
