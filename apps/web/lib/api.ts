...
import type { AuditData, AuditEvent, AuditFilters, ConsoleData, CustomerProfile, Decision, DecisionResponse, Evidence, OrderTracking, PurchaseResult, QueueFilters, Run, Ticket, TicketDraft, TicketWorkItem } from "@/lib/types";
...
export async function getConsoleData(offset = 0, limit = 50, filters: QueueFilters = { sort: "newest" }): Promise<ConsoleData> {
  ...
  try {
    const payload = await request<{
      items: { ticket: ApiTicket; run?: ApiRun }[];
      total: number;
      offset: number;
      limit: number;
    }>(`/api/v1/ticket-queue?${queueParams(offset, limit, filters)}`, "admin");
    ...
    return {
      ...
      source: "unavailable",
      checkedAt,
      loadError: error instanceof Error ? error.message : "Operations API is unavailable",
      total: 0,
      offset,
      limit,
    };
...
const toCustomer = (value: ApiCustomer): CustomerProfile => ({ id: value.id, name: value.name, email: value.email, phone: value.phone, createdAt: value.created_at, updatedAt: value.updated_at });

export async function registerCustomer(input: { name: string; email: string; phone: string; password: string }) {
  return toCustomer(await request<ApiCustomer>("/api/v1/customer/register", "customer", { method: "POST", body: JSON.stringify(input) }));
}

export async function loginCustomer(input: { email: string; password: string }) {
  return toCustomer(await request<ApiCustomer>("/api/v1/customer/login", "customer", { method: "POST", body: JSON.stringify(input) }));
}

export async function updateCustomer(input: { name: string; phone: string }) {
  return toCustomer(await request<ApiCustomer>("/api/v1/customer/me", "customer", { method: "PATCH", body: JSON.stringify(input) }));
}
...
type ApiAuditEvent = {
  id: string; occurred_at: string; actor_id: string; actor_type: AuditEvent["actorType"];
  action: string; resource_type: string; resource_id?: string; outcome: AuditEvent["outcome"];
  request_id?: string; metadata: Record<string, unknown>;
};

function toAuditEvent(event: ApiAuditEvent): AuditEvent {
  return {
    id: event.id, occurredAt: event.occurred_at, actorId: event.actor_id, actorType: event.actor_type,
    action: event.action, resourceType: event.resource_type, resourceId: event.resource_id,
    outcome: event.outcome, requestId: event.request_id, metadata: event.metadata,
  };
}
