...
const cases = await Promise.all(rows.map(async (row) => {
  ...
  return { id: row.id, retrieved: retrieved.map((doc) => doc.id), recall_at_k: relevant ? 1 : 0, reciprocal_rank: rank >= 0 ? 1 / (rank + 1) : 0, citation_correct: Boolean(relevant?.page_number && row.relevant_pages.includes(relevant.page_number) && relevant.citation) };
}));
